...
int main()
{
    ...
    if(f && !vanpoztiv(db,db1, oszlop_id))
    {
...
void kiir( int db, int db1,int oszlop_id,int sor_id)
{
    ...
    for(int j=0; j<db1; j++)
        {
            if(oszlop[j][0]=='y') SetColor(1);
            if(oszlop[j][0]=='x') SetColor(14);
            cout<< oszlop[j];
            SetColor(15);
            cout.width(10);
        }
